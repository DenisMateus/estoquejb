  ...
  return (
    <AppLayout>
      <div className="space-y-4">
        ...
        {/* Resumo do setor */}
        <div className="bg-card border rounded-lg p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div>
            <div className="text-xs text-muted-foreground">Itens cadastrados</div>
            <div className="text-lg font-bold font-mono text-foreground">{products.length}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground">Itens (filtro)</div>
            <div className="text-lg font-bold font-mono text-foreground">{filtered.length}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground">Estoque total (un/barras)</div>
            <div className="text-lg font-bold font-mono text-primary">
              {formatQuantity(products.reduce((s, p) => s + (p.quantity || 0), 0))}
            </div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground">Peso total (kg)</div>
            <div className="text-lg font-bold font-mono text-primary">
              {products.reduce((s, p) => s + (p.quantity || 0) * (p.weightPerUnit || 0), 0).toFixed(2)}
            </div>
          </div>
        </div>
